...
class DirkAST {
  ...
  DirkAST({
    String start = '',
    String layout = '@renderBody()',
    String contents,
    this.isPartial = false,
  })  : this.start = start,
        this.contents = isPartial
            ? contents
            : "${start.length > 0 ? start + '\n' : ''}${layout.replaceAll(RegExp(r'@renderBody\(\)'), contents)}";
  ...
  String toViewFunction(String name) {
    ...
    Token model =
        tree.firstWhere((el) => el.type == TokenType.model, orElse: () => null);
    tree.removeWhere(
        (el) => el.type == TokenType.$import || el.type == TokenType.model);
    String modelType = model?.content ?? "";

    result.writeln("// Generated by DirkAST, don't modify");
    result.writeln("");
    result.writeln("// ignore_for_file: unused_import, unused_local_variable");
    result.writeln("import 'views.dirk.dart';");
    ...
    result.writeln("return res;");
    result.writeln("}");

    return tryFormatCode(result.toString());
  }
}
